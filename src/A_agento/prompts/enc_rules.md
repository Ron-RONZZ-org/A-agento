...
- `difino` and `semantika` are complementary and should NOT repeat same information twice
  - e.g, if `semantika` includes `str wdt:P1451 "Liberte, Egalite, Fraternite"`, `difino` should NOT remention "Liberte, Egalite, Fraternite". The only exception is that if further explanation is done in `difino` on the republican slogan, and repetition becomes therefore inevitable
