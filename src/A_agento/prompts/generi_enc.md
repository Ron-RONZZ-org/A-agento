...
4. WORKFLOW for creatin inline semantic links in `difino.{{lang}}` (use MCP tools):
...
6. **`semantika` is for numerical values and literal string, whereas `difino` is for explanatory text**
...
7. LANGUAGES
  - `terminologio` should be in 3 default languages: `eo`, `fr`, `en` + the original language of the concept
